...
cat /kafka/config/server.properties.template | sed \
  ...
  -e "s|{{KAFKA_LISTENERS}}|${KAFKA_LISTENERS:-PLAINTEXT://$IP:9192,CONTROLLER://$IP:9093,LISTENER_DOCKER_EXTERNAL://$IP:9092}|g" \
  -e "s|{{KAFKA_ADVERTISED_LISTENERS}}|${KAFKA_ADVERTISED_LISTENERS:-PLAINTEXT://$IP:9192,LISTENER_DOCKER_EXTERNAL://$IP:9092}|g" \
